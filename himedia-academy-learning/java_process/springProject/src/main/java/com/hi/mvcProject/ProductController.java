...
public class ProductController {
	...
	public void plus(ProductVO vo,Model model, HttpSession session) {
		// 기존에 있는 장바구니 세션을 가지고 와서, 물건 개수: 10개 였다면
		// 지역변수는 반드시 초기화해주는 습관을 가지자!!!
		// 기본형(값): 정수, 실수, 문자1, 논리
		// 기본형 초기화: 해당 타입의 값으로 초기화
		// 참조형(주소): 객체, 클래스, 배열, ...
		// 참조형 초기화: null로 초기화
		// 변수는 언제 생기는가? --> 변수타입 변수명; // 변수 선언 시 생성!!
		...
		if(session.getAttribute("basket") != null) {
			...
			// 한 번 장바구니에 넣은 적이 없어서, 세션으로 잡혀있는 것 없음
			// 리스트를 만들어주어야 함.
			list = new ArrayList<ProductVO>();
		}
		// 새로운 물건을 장바구니 세션 목록에 추가, 물건 개수: 11개
		list.add(vo);
		// 세션을 새로운 목록으로 업데이트, 세션에는 10개이므로 11개로 업데이트
		// session의 값 변수의 타입은 Object로 되어있음
		// 어떤 타입이든 모두가 다 Object의 상속이므로
		// 모든 타입들을 Object으로 선언된 변수에 넣을 수 있다.
		// String(작은, 아래, 자식) -> Object(큰, 위, 최상위 부모)
		// 자동타입변환(자동형변환), 업캐스팅
		// ArrayList -> Object
