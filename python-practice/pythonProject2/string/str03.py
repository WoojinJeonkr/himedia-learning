today = '오늘은 목요일입니다. 코로나 검사해야 합니다.'

print(today.find('오늘')) # 0
print(today.find('오')) # 0
print(today.find('목요일')) # 4
print(today.find('코로나')) # 12 : 시작 인덱스
print(today.find('독감')) # -1 : 없음

print(today.count('코로나')) # 1
...
print(id.upper() == 'ROOT') # upper(): 대문자로
print(id.lower() == 'root') # lower(): 소문자로
...
for i in range(0, len(news3), 1):
    # print(news3[i])
    # if(news3[i] == '주꾸미'): # '주꾸미'라는 단어와 완전히 같은 단어
    # if(news3[i].startswith('주꾸미')): # '주꾸미'로 시작하는 단어
    if(news3[i].endswith('주꾸미')): # '주꾸미'로 끝나는 단어
        print('찾음>> ', i)
        ju_list.append(i)
        count += 1 # count = count + 1
print('최종 쭈꾸미 개수는 ', count)
